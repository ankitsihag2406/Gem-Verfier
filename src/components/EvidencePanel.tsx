...
import { ExtractedParameter } from "@/lib/mockData";
...
interface Props { parameter: ExtractedParameter | null; onClose: () => void; }

// ── Confidence quality indicators ────────────────────────────────────────────
function qualityIndicators(param: ExtractedParameter) {
...
// ── Chain step ───────────────────────────────────────────────────────────────
function ChainStep({
  step, label, children, isLast = false,
}: { step: number; label: string; children: React.ReactNode; isLast?: boolean }) {
  return (
    <div style={{ display: "flex", gap: 12 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
        ...
        {!isLast && (
          <div style={{ width: 1, flex: 1, background: "#DBEAFE", minHeight: 16, marginTop: 4 }} />
        )}
      </div>
...
function EmptyState() {
  return (
    <div style={{
      ...
      justifyContent: "center", padding: "48px 24px", textAlign: "center", height: "100%",
    }}>
      <div style={{
        width: 48, height: 48, borderRadius: 10, background: "#EFF6FF",
        display: "flex", alignItems: "center", justifyContent: "center",
        marginBottom: 14, border: "1px solid #BFDBFE",
      }}>
        <Quote size={20} color="var(--gem-navy)" />
      </div>
      <div style={{ fontSize: 13.5, fontWeight: 700, color: "var(--text-heading)", marginBottom: 6 }}>
        Evidence Chain
      </div>
...
// ── Main component ────────────────────────────────────────────────────────────
export default function EvidencePanel({ parameter, onClose }: Props) {
  ...
  const isMissing   = parameter.status === "NOT_FOUND";
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      <div style={{
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{
            fontSize: 9.5, fontWeight: 700, color: "var(--gem-navy)",
            textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 3,
          }}>
            Evidence Chain · AI Extraction
          </div>
          <div style={{ fontSize: 13, fontWeight: 700, color: "var(--text-heading)", lineHeight: 1.3 }}>
          ...
          <div className="alert alert-warn" style={{ marginBottom: 16 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 12, lineHeight: 1.5 }}>
                AI confidence is below 80%. This parameter has been flagged for manual verification before a compliance decision is issued.
              </div>
              <button style={{
        ...
        <div>
          ...
          <ChainStep step={1} label="Tender Requirement">
            <div style={{
              ...
              borderRadius: 5,
              fontSize: 12.5, color: "var(--text-body)", lineHeight: 1.6,
            }}>
              {parameter.requirement}
            </div>
          </ChainStep>

          {/* Step 2 — Bidder Evidence */}
          ...
          <ChainStep step={3} label="Document Reference">
            <div style={{
              ...
              <div style={{
                ...
                <span style={{ fontSize: 12, color: "var(--text-body)", fontFamily: "monospace" }}>
                  bidder_submission.pdf
                </span>
              </div>
              {parameter.pageNumber && (
                <div style={{
                  ...
                  <span style={{ fontSize: 12, fontWeight: 700, color: "var(--gem-navy)" }}>
          ...
          <ChainStep step={4} label="AI Extracted Value">
            ...
            {confPct !== null && (
              <div style={{
                padding: "10px 12px",
                background: "#F8FAFC", border: "1px solid var(--border-light)", borderRadius: 5,
              }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 7 }}>
                  <span style={{ fontSize: 11.5, color: "var(--text-secondary)" }}>{confLabel}</span>
                  <span style={{ fontSize: 18, fontWeight: 800, color: confColor, lineHeight: 1 }}>
                    {confPct}%
                  </span>
                </div>
                {/* Bar */}
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  {indicators.map(({ label, ok }) => (
                    <div key={label} style={{ display: "flex", alignItems: "center", gap: 7, fontSize: 12 }}>
                      ...
                      <span style={{ color: ok ? "#15803D" : "#DC2626", fontWeight: ok ? 600 : 400 }}>
                        {label}
                      </span>
                    </div>
                  ))}
          ...
          <ChainStep step={5} label="Compliance Decision" isLast>
            {(() => {
              const s = parameter.status;
              const map = {
                COMPLIANT:       { bg: "#F0FDF4", border: "#BBF7D0", color: "#15803D", label: "Requirement Satisfied", Icon: CheckCircle2 },
                NON_COMPLIANT:   { bg: "#FEF2F2", border: "#FECACA", color: "#DC2626", label: "Requirement Failed",    Icon: XCircle },
              ...
              const { bg, border, color, label, Icon } = map[s] ?? map.PENDING;
              return (
                <div style={{
                  padding: "11px 14px",
                  ...
                  {confPct !== null && (
                    <span style={{
                      marginLeft: "auto", fontSize: 11.5, color,
                      fontWeight: 700, background: "rgba(0,0,0,0.05)",
              ...
              <div style={{ marginTop: 10 }}>
                <button
                  ...
                  style={{
                    ...
                    padding: "7px 10px",
                    background: "#F8FAFC", border: "1px solid var(--border-light)",
                    borderRadius: showReasoning ? "5px 5px 0 0" : 5,
                ...
                {showReasoning && (
                  <div style={{
                    padding: "10px 12px",
                    background: "#fff", border: "1px solid var(--border-light)", borderTop: "none",
                    borderRadius: "0 0 5px 5px", fontSize: 12.5, color: "var(--text-body)", lineHeight: 1.7,
                  }}>
                    The AI engine located the relevant clause on page {parameter.pageNumber} using semantic matching against the tender requirement.
                    The extracted value of <strong>{parameter.extractedValue}</strong> was compared against the threshold.
                    {parameter.confidenceScore && parameter.confidenceScore >= 0.8
                      ? " High lexical similarity between the source text and requirement indicates a strong match."
                      : " The language in the document was ambiguous — the extracted value may require verification against certified documents."}
                  </div>
                )}
              </div>
            )}
