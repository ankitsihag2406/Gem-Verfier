...
export default function NewTenderPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    router.push(`/tenders/${MOCK_TENDERS[0].id}`);
  };
    ...
    <AppShell>
      ...
      <div className="page-content" style={{ maxWidth: 680 }}>
        ...
        <div className="alert alert-info" style={{ marginBottom: 20 }}>
          ...
          <div>
            Define the AI verification rules for this tender. The system will automatically extract and validate these three parameters from every submitted bid. Additional documents can be manually reviewed.
          </div>
        </div>

        <form onSubmit={handleSubmit}>
          {/* Section 1: Tender Identity */}
          ...
          <div className="card" style={{ marginBottom: 20 }}>
            ...
            <div className="card-body" style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <p style={{ fontSize: 13, color: "var(--text-secondary)", lineHeight: 1.6 }}>
                These three parameters will be automatically extracted from each submitted bid PDF and verified against the values below using AI.
              </p>
              <div>
                <label className="field-label">EMD Required (₹) <span style={{ color: "var(--status-red)" }}>*</span></label>
                <input className="input-field" type="number" value={form.emdRequired}
                  onChange={e => setForm(f => ({ ...f, emdRequired: e.target.value }))} placeholder="500000" required />
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                <div>
                  ...
                  <input className="input-field" type="number" value={form.minExperienceYears}
                    onChange={e => setForm(f => ({ ...f, minExperienceYears: e.target.value }))} placeholder="5" required />
                ...
                <div>
                  ...
                  <input className="input-field" type="number" value={form.minAnnualTurnover}
                    onChange={e => setForm(f => ({ ...f, minAnnualTurnover: e.target.value }))} placeholder="10000000" required />
          ...
          <div style={{ display: "flex", gap: 12 }}>
            <button type="submit" className="btn btn-primary" style={{ flex: 1, justifyContent: "center", padding: "11px" }}>
              Create Tender & Accept Bids
            </button>
            <Link href="/tenders" className="btn btn-ghost" style={{ padding: "11px 20px" }}>
