...
import {
  MOCK_TENDERS, MOCK_BIDS, formatCurrency, formatDate,
  getComplianceSummary, formatProcessingTime,
} from "@/lib/mockData";
import {
...
function getDashboardStats() {
  const completedBids = MOCK_BIDS.filter(b => b.status === "COMPLETE");
  const verified  = completedBids.filter(b => b.overallCompliance === "COMPLIANT").length;
  const nonComp   = completedBids.filter(b => b.overallCompliance === "NON_COMPLIANT").length;
  const review    = completedBids.filter(b => b.overallCompliance === "PARTIAL").length;
  const failed    = MOCK_BIDS.filter(b => b.status === "FAILED").length;

  const totalReqs = completedBids.reduce((acc, b) => {
    const s = getComplianceSummary(b.parameters);
    return acc + s.aiVerified;
  }, 0);

  const avgTime = Math.round(
    completedBids
      .filter(b => b.processingTimeSec !== null)
      .reduce((acc, b) => acc + (b.processingTimeSec ?? 0), 0) /
    completedBids.filter(b => b.processingTimeSec !== null).length
  );

  const now = new Date();
  const syncTime = now.toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" }) + " IST";
  const syncDate = now.toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" });

  return { verified, nonComp, review, failed, totalReqs, avgTime, syncTime, syncDate };
}
...
function ConfPill({ value }: { value: number | null }) {
  ...
  return (
    <span style={{
      ...
      {value < 0.80 && <AlertTriangle size={10} />}
      {pct}%
    </span>
  );
}
    ...
    <AppShell>
      ...
      <div className="page-bar">
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <Link href="/tenders/new" className="btn btn-orange btn-sm">
      ...
      <div className="page-content">
        ...
        <div style={{
          ...
          borderLeft: "4px solid var(--gem-navy)",
          borderRadius: 6, padding: "12px 18px",
          marginBottom: 18, gap: 24, alignItems: "center",
        }}>
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, color: "var(--gem-navy)", textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 4 }}>
            ...
            <div style={{ fontSize: 13.5, color: "var(--text-body)", lineHeight: 1.5 }}>
              ...
              Avg processing time: <strong style={{ color: "var(--gem-navy)" }}>{formatProcessingTime(stats.avgTime)}</strong>
            </div>
          </div>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 3, borderLeft: "1px solid var(--border-light)", paddingLeft: 20 }}>
            ...
            <div style={{ fontSize: 11, color: "var(--text-muted)" }}>
              Last synchronised: {stats.syncDate}, {stats.syncTime}
            </div>
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginBottom: 18 }}>
          {[
            ...
            <div key={label} style={{
              ...
              borderRadius: 7, padding: "14px 16px", borderTop: `3px solid ${color}`,
            }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: 16 }}>
          ...
          <div className="card">
            <div className="card-header">
              <div className="card-header-title">
                <Activity size={15} /> Bid Verification Results
              </div>
              <Link href="/tenders" style={{ fontSize: 12.5, color: "var(--gem-navy)", fontWeight: 600 }}>
                All Tenders →
              </Link>
            </div>
              ...
              <tbody>
                {MOCK_BIDS.map(bid => {
                  ...
                  if (!isFailed && summary.aiVerified > 0) {
                    if (bid.overallCompliance === "COMPLIANT")
                      finding = `${summary.compliant}/${summary.aiVerified} requirements satisfied`;
                    else if (bid.overallCompliance === "NON_COMPLIANT")
                      finding = `${summary.nonCompliant} failed${summary.flagged > 0 ? ` · ${summary.flagged} missing` : ""}`;
                    else
                      finding = `${summary.flagged} low-confidence · manual review`;
                  }
                      ...
                      <td>
                        ...
                        {bid.processingTimeSec && (
                          <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 2 }}>
                            ...
                            Processed in {formatProcessingTime(bid.processingTimeSec)}
                          </div>
                        )}
                      </td>
                      <td>
                        ...
                        {isFailed && (
                          <div style={{
                            fontSize: 10.5, color: "#DC2626", fontWeight: 600,
                            marginTop: 2, display: "flex", alignItems: "center", gap: 3,
                          }}>
                            <AlertOctagon size={9} /> DPI &lt; 150 — illegible scan
                          </div>
                        )}
                      </td>
                      <td><span style={{ fontSize: 12, color: "var(--text-secondary)" }}>{formatDate(bid.uploadedAt)}</span></td>
                      <td style={{ textAlign: "center" }}>
                        <ConfPill value={bid.avgConfidence} />
                      </td>
                      <td>
                        <span style={{ fontSize: 12, color: isFailed ? "#DC2626" : isReview ? "#B45309" : "var(--text-body)" }}>
                          {finding}
                        </span>
                      </td>
                      <td>
                      ...
                      <td>
                        {!isFailed ? (
                          ...
                          <span style={{
                            ...
                            background: "#DC2626", padding: "3px 10px",
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div className="card">
              <div className="card-header">
                <div className="card-header-title">
                  <FileText size={15} /> Active Tenders
                </div>
              </div>
              {MOCK_TENDERS.map(t => {
                ...
                return (
                  <div key={t.id} style={{ padding: "11px 14px", borderBottom: "1px solid var(--border-light)" }}>
                    <div style={{ fontSize: 12.5, fontWeight: 700, color: "var(--text-heading)", marginBottom: 3, lineHeight: 1.3 }}>
                      {t.name}
                    </div>
                    ...
                    <div className="progress-track">
                      <div className="progress-fill" style={{ width: `${pct}%`, background: "#15803D" }} />
                    </div>
                    <div style={{ marginTop: 7 }}>
                      <Link href={`/tenders/${t.id}`} className="row-action" style={{ fontSize: 11.5 }}>
                        Open Verification →
                      </Link>
                    </div>
                  </div>
            ...
            <div style={{
              ...
              background: "#F8FAFC",
              border: "1px solid var(--border-light)",
              borderLeft: "3px solid var(--gem-navy)",
              borderRadius: 6,
            }}>
              ...
              <Link href="/activity" style={{ fontSize: 12, color: "var(--gem-navy)", fontWeight: 600 }}>
                View Full Audit Log →
              </Link>
            </div>
          </div>
