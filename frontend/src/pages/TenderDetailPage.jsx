      ...
      <div className="page-content">
        <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 18, alignItems: "start" }}>
              ...
              <div className="card-body">
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12 }}>
                  {[
                    ...
                    <div key={label} style={{ padding: "12px 14px", background: "#F8FAFC", border: "1px solid var(--border-light)", borderRadius: 6 }}>
                      <div style={{ fontSize: 11, color: "var(--text-muted)", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 5 }}>{label}</div>
            ...
            <div className="card">
              <div className="card-header">
                <div className="card-header-title">
                  Submitted Bids <span className="chip" style={{ marginLeft: 4 }}>{bids.length}</span>
                </div>
              </div>
              {bids.length === 0 ? (
                <div style={{ padding: "40px 20px", textAlign: "center", color: "var(--text-muted)", fontSize: 13 }}>No bids submitted yet.</div>
              ) : (
                <table className="data-table">
                  <thead>
                    <tr><th>Company</th><th>PDF File</th><th>Submitted</th><th>Result</th><th></th></tr>
                  </thead>
                  <tbody>
                    {bids.map(bid => {
                      ...
                      return (
                        <tr key={bid.id}>
                          ...
                          <td>
                            <Link to={`/bids/${bid.id}`} className="row-action">View Analysis <ArrowRight size={12} /></Link>
                          </td>
                        </tr>
                      );
                    })}
          ...
          <div className="card" style={{ position: "sticky", top: 80 }}>
            <div className="card-header">
              <div className="card-header-title"><Upload size={15} /> Submit New Bid</div>
            </div>
            <div className="card-body">
              <p style={{ fontSize: 13, color: "var(--text-secondary)", marginBottom: 14, lineHeight: 1.6 }}>
                Upload a bidder&apos;s proposal PDF. The AI engine will automatically extract and verify all compliance parameters.
              </p>
