  ...
  return (
    <div style={{ height: "100%", overflowY: "auto", background: "#E8EDF2" }}>
      <div className="pdf-panel-header">
        ...
        {highlightQuote && (
          <span style={{ fontSize: 11, padding: "2px 8px", background: "#FFFBEB", border: "1px solid #FDE68A", borderRadius: 4, color: "#B45309", fontWeight: 600 }}>
            ● Evidence Highlighted
          </span>
        )}
      </div>
      <div style={{ padding: "16px 24px" }}>
        <div style={{ background: "#fff", boxShadow: "0 2px 12px rgba(0,0,0,0.12)", borderRadius: 2, padding: "40px 48px", fontFamily: "Georgia, 'Times New Roman', serif", fontSize: 13, lineHeight: 1.8, color: "#1a1a1a", minHeight: "100%" }}>
          <div style={{ borderBottom: "3px solid #003366", paddingBottom: 16, marginBottom: 24, display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
            <div>
              <div style={{ fontSize: 17, fontWeight: 700, color: "#003366" }}>{bid.companyName}</div>
              <div style={{ fontSize: 11.5, color: "#555", marginTop: 3 }}>CIN: U72200HR2016PTC062418 · GSTIN: 06AABCA1234M1Z5</div>
            </div>
          ...
          <div style={{ textAlign: "center", marginBottom: 28, paddingBottom: 20, borderBottom: "1px solid #E5E7EB" }}>
            <div style={{ fontSize: 10, color: "#888", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 4 }}>GOVERNMENT OF INDIA · Government e-Marketplace (GeM)</div>
          ...
          {sections.map((sec, idx) => {
            ...
            return (
              <div key={idx} id={isHighlighted ? "highlighted-quote" : undefined} style={{ marginBottom: 24, background: isHighlighted ? "#FFFBEB" : "transparent", borderLeft: isHighlighted ? "4px solid #F59E0B" : "none", padding: isHighlighted ? "10px 14px" : "0", borderRadius: isHighlighted ? "0 6px 6px 0" : 0, transition: "background 0.3s" }}>
      ...
      <div style={{ flex: 1, overflow: "hidden", display: "flex", flexDirection: "column", height: "100%" }}>
        ...
        <div className="page-bar" style={{ flexShrink: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <Link to={`/tenders/${bid.tenderId}`} style={{ display: "flex", alignItems: "center", gap: 5, color: "var(--text-secondary)", fontSize: 13, fontWeight: 500 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            {bid.processingTimeSec && (
              <div style={{ fontSize: 11.5, color: "var(--text-secondary)", padding: "3px 10px", background: "#F8FAFC", border: "1px solid var(--border-light)", borderRadius: 4 }}>
                Processed in {formatProcessingTime(bid.processingTimeSec)}
              </div>
            )}
            {bid.avgConfidence !== null && (
              <div style={{ fontSize: 12, fontWeight: 700, color: bid.avgConfidence >= 0.85 ? "#15803D" : "#B45309", padding: "3px 10px", background: bid.avgConfidence >= 0.85 ? "#F0FDF4" : "#FFFBEB", border: `1px solid ${bid.avgConfidence >= 0.85 ? "#BBF7D0" : "#FDE68A"}`, borderRadius: 4 }}>
                Avg Confidence: {Math.round(bid.avgConfidence * 100)}%
              </div>
            )}
            <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "4px 12px", background: overallColor + "14", border: `1px solid ${overallColor}30`, borderRadius: 20, fontSize: 12.5, fontWeight: 700, color: overallColor }}>
              <OverallIcon size={13} />{bid.overallCompliance.replace("_", " ")}
            </div>
            ...
            {bid.overallCompliance === "PARTIAL" && (
              <button style={{ display: "flex", alignItems: "center", gap: 5, padding: "5px 12px", fontSize: 12, fontWeight: 700, background: "#B45309", color: "#fff", border: "none", borderRadius: 5, cursor: "pointer" }}>
        ...
        <div style={{ flex: 1, display: "grid", gridTemplateColumns: "1fr 1fr 340px", overflow: "hidden" }}>
          ...
          <div style={{ borderRight: "1px solid var(--border-light)", overflow: "hidden", display: "flex", flexDirection: "column" }}>
            <div className="pdf-panel-header" style={{ flexShrink: 0 }}>
              ...
              <span style={{ fontWeight: 600, color: "var(--text-heading)", fontSize: 12.5 }}>Bid Document</span>
              <span style={{ marginLeft: "auto", fontSize: 11, color: "var(--text-muted)" }}>Click a parameter → jumps to evidence</span>
            </div>
          ...
          <div style={{ borderRight: "1px solid var(--border-light)", overflow: "hidden", display: "flex", flexDirection: "column" }}>
            <div className="pdf-panel-header" style={{ flexShrink: 0 }}>
              <Brain size={13} color="var(--gem-navy)" />
              <span style={{ fontWeight: 600, color: "var(--text-heading)", fontSize: 12.5 }}>AI Compliance Matrix</span>
              {tender && (
                <span className="mono" style={{ marginLeft: "auto", fontSize: 11, color: "var(--text-muted)" }}>
                  <Hash size={10} style={{ display: "inline" }} /> {tender.tenderRef}
                </span>
              )}
            </div>
